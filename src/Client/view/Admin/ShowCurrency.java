...
public class ShowCurrency {
    ...
    void OnEditButtonPressed(ActionEvent event) throws IOException {
        if (Currency.getSelectionModel().getSelectedItem() != null) {
            Currency dlg = new Currency();
            dlg.setCurrencyID(Currency.getSelectionModel().getSelectedItem().getCurrencyID());
            dlg.setCurrencyName(Currency.getSelectionModel().getSelectedItem().getCurrencyName());
            dlg.setRoundingStep(Currency.getSelectionModel().getSelectedItem().getRoundingStep());
            dlg.setCurrencyRate(Currency.getSelectionModel().getSelectedItem().getCurrencyRate());
            ObjectOutputStream file = new ObjectOutputStream(new FileOutputStream("C:\\Users\\Admin\\IdeaProjects\\CP (client)\\src\\files\\editcurrency.dat"));
            ...
            loader.setLocation(getClass().getResource("/Client/view/Admin/Editing/EditCurrency.fxml"));
            try {
                loader.load();
            } catch (IOException e) {
                e.printStackTrace();
    ...
    void OnConvertPressed(ActionEvent event) {
        ...
        stage.close();
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/Client/view/Converter.fxml"));
        Parent root1 = null;
        try {
            root1=(Parent) fxmlLoader.load();
        } catch (IOException e) {
